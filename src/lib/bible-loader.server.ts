// src/lib/bible-loader.server.ts
...
const ALIASES: Record<string, string> = {
  ...
  'manasseh':'man','prayer-of-manasseh':'man','1-esdras':'1es','2-esdras':'2es',
  'esther-greek':'esg','additions-to-esther':'esg','letter-of-jeremiah':'lje',
  'song-of-three-jews':'s3y','azariah':'s3y','susanna':'sus','bel-dragon':'bel',
  'bel-and-the-dragon':'bel','wisdom-of-solomon':'wis','ecclesiasticus':'sir',
...
export async function loadChapter(book: string, chapter: number, lang: string = 'hin-hindi'): Promise<Verse[] | null> {
  ...
  try {
    ...
    if (res.ok) { const b: BibleBook = await res.json(); return b[String(chapter)] || b[chapter] || null; }
  ...
  try {
    ...
    const bd = bible[code];
    if (!bd) return null;
    return bd[String(chapter)] || bd[chapter] || null;
  } catch (e) { console.error('❌ Combined error:', e); return null; }
  return null;
}

export async function loadBook(book: string, lang: string = 'hin-hindi'): Promise<BibleBook | null> {
  const code = getCode(book);
...
export async function loadForGemini(book: string, chapter: number, options: { includeVerseNumbers?: boolean; format?: 'plain'|'json'|'markdown'; lang?: string } = {}): Promise<string> {
